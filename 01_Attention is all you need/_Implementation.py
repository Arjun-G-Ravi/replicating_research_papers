...
class TransformerBlock(nn.Module):
    ''' Refers to the upper part of the encoder block, on the left side of the transformer.'''
    ...
    def forward(self, value, key, query, mask):
        ...
        # x is for the resnet connection
...
class Transformer(nn.Module):
    ...
    def make_src_mask(self, src): # Source mask
        src_mask = (src != self.src_pad_idx).unsqueeze(1).unsqueeze(2)
        # (N, 1, 1, src_len)
        return src_mask.to(self.device)

    def make_trg_mask(self, trg): # Source mask
        N, trg_len = trg.shape
